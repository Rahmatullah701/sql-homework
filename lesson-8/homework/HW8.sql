...
FROM (
    SELECT Year, district_id, Population
    FROM City_Population
    WHERE district_id IN ('Bektemir', 'Chilonzor', 'Yakkasaroy')
) AS SourceTable
PIVOT (
    SUM(Population) FOR district_id IN ([Bektemir], [Chilonzor], [Yakkasaroy])
) AS PivotTable;
